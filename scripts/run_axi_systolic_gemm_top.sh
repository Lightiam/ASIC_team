...
echo "===== Existing AXI command-path regression compilation ====="
iverilog \
    -g2012 \
    -Wall \
...
echo "===== Existing AXI command-path regression ====="
vvp build/tb_nce_axi_mixed_precision_top.vvp \
    | tee reports/tb_nce_axi_mixed_precision_top.log

echo "===== AXI native systolic-GEMM compilation ====="
iverilog \
    -g2012 \
    -Wall \
    -s tb_nce_axi_systolic_gemm_top \
    -o build/tb_nce_axi_systolic_gemm_top.vvp \
    "${RTL_FILES[@]}" \
    tb/integration/tb_nce_axi_systolic_gemm_top.sv

echo "===== AXI native systolic-GEMM simulation ====="
vvp build/tb_nce_axi_systolic_gemm_top.vvp \
    | tee reports/tb_nce_axi_systolic_gemm_top.log

echo "===== Verilator integrated-top lint ====="
verilator \
    --lint-only \
    --timing \
    -Wall \
...
echo "===== Yosys integrated-top synthesis check ====="
yosys -q -p "
    ...
    check -assert;
    stat;
" | tee reports/yosys_nce_axi_systolic_gemm_top.log

echo "===== All AXI systolic-GEMM integration checks completed ====="
