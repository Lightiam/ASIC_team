...
// -----------------------------------------------------------------------------
// Mixed-precision register-to-MAC execution core.
//
// Integrates:
//   - 16 x 256-bit vector register bank
//   - 16 x 256-bit matrix register bank
//   - Eight-lane mixed-precision SIMD engine
//
...
//   2'b10: BF24 multiply
//
// Each execution transaction reads one vector register and one matrix register.
// The selected operation updates the same eight architectural FP32
// accumulators.
// -----------------------------------------------------------------------------

module nce_mixed_precision_register_mac_core #(
    ...
    input  logic [1:0]   exec_precision_i,
    ...
    logic simd_ready;
    logic simd_valid;
    logic simd_precision_supported;
    ...
    // Operand errors only describe invalid register sources.
    // Unsupported precision is reported separately.
    assign operand_error_o =
        exec_valid_i &&
        !operand_valid_o &&
        !register_clear_i;

    nce_mixed_precision_simd8_mac #(
        ...
        .precision_i           (exec_precision_i),
        .precision_supported_o (simd_precision_supported),
