...
module tb_nce_fp32_addsub_raw;
    ...
    logic [27:0] expected_significand;
    ...
    initial begin
        ...
        vector_file =
            $fopen("build/fp32_addsub_raw_vectors.txt", "r");

        if (vector_file == 0) begin
            $fatal(
                1,
                "Could not open build/fp32_addsub_raw_vectors.txt"
            );
        end
        ...
        while (!$feof(vector_file)) begin
            ...
            if (scan_result == 6) begin
                ...
                if (
                    result_sign !== expected_sign ||
                    result_exponent !== expected_exponent ||
                    result_significand !== expected_significand ||
                    exact_zero !== expected_exact_zero
                ) begin
                    error_count = error_count + 1;

                    if (error_count <= 20) begin
                        $display(
                            "ERROR a=%08h b=%08h sign=%h/%h exponent=%02h/%02h significand=%07h/%07h zero=%h/%h",
                            vector_a,
                            vector_b,
                            result_sign,
                            expected_sign,
        ...
        if (error_count == 0) begin
            $display(
                "PASS: nce_fp32_addsub_raw passed all %0d vectors.",
                test_count
            );
        end
        else begin
