...
run_testbench() {
    ...
    echo "===== ${description} compilation ====="

    iverilog \
        -g2012 \
        -Wall \
        -s "${top_name}" \
        -o "build/${output_name}.vvp" \
        "${RTL_FILES[@]}" \
        "${tb_file}"

    echo "===== ${description} simulation ====="

    vvp "build/${output_name}.vvp" \
        | tee "reports/${output_name}.log"
}

run_testbench \
    tb_nce_systolic_gemm_4x4 \
    tb/integration/tb_nce_systolic_gemm_4x4.sv \
    tb_nce_systolic_gemm_4x4 \
    "Autonomous native 4x4 GEMM"

run_testbench \
    tb_nce_tiled_gemm_8x8 \
    tb/integration/tb_nce_tiled_gemm_8x8.sv \
    tb_nce_tiled_gemm_8x8 \
    "Autonomous 8x8 tiled GEMM"

run_testbench \
    tb_nce_axi_mixed_precision_top \
    tb/integration/tb_nce_axi_mixed_precision_top.sv \
    tb_nce_axi_mixed_precision_top \
    "Legacy AXI mixed-precision path"

run_testbench \
    tb_nce_axi_systolic_gemm_top \
    tb/integration/tb_nce_axi_systolic_gemm_top.sv \
    tb_nce_axi_systolic_gemm_top \
    "AXI native 4x4 systolic GEMM"

run_testbench \
    tb_nce_axi_tiled_gemm_8x8_top \
    tb/integration/tb_nce_axi_tiled_gemm_8x8_top.sv \
    tb_nce_axi_tiled_gemm_8x8_top \
    "AXI 8x8 tiled GEMM"

echo "===== Verilator complete-top lint ====="
...
echo "===== Yosys complete-top synthesis check ====="
...
yosys -q -p "
    ...
    check -assert;
    stat;
" | tee reports/yosys_nce_axi_tiled_gemm_8x8_top.log

echo "===== All AXI 8x8 tiled-GEMM integration checks completed ====="
