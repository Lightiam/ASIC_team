...
RTL_FILES=(
    ...
    # Native systolic hierarchy
    rtl/core/nce_mixed_precision_mac_lane.sv
    rtl/core/nce_systolic_pe.sv
    rtl/core/nce_systolic_array_4x4.sv
)

TB_FILE="tb/integration/tb_nce_systolic_array_4x4.sv"

echo "===== Icarus compilation ====="
iverilog \
    -g2012 \
    -Wall \
    -s tb_nce_systolic_array_4x4 \
    -o build/tb_nce_systolic_array_4x4.vvp \
    "${RTL_FILES[@]}" \
    "${TB_FILE}"

echo "===== Native 4x4 systolic GEMM simulation ====="
vvp build/tb_nce_systolic_array_4x4.vvp \
    | tee reports/tb_nce_systolic_array_4x4.log
...
verilator \
    ...
    -Wno-fatal \
    -Wno-PINCONNECTEMPTY \
    --top-module nce_systolic_array_4x4 \
    "${RTL_FILES[@]}"

echo "===== Yosys synthesis check ====="
yosys -q -p "
    read_verilog -sv ${RTL_FILES[*]};
    hierarchy -check -top nce_systolic_array_4x4;
    proc;
    opt;
    check -assert;
    stat;
" | tee reports/yosys_nce_systolic_array_4x4.log

echo "===== All native 4x4 systolic-array checks completed ====="
