...
// -----------------------------------------------------------------------------
// Command-controlled mixed-precision NCE execution core.
//
// Supported commands:
//
//   DOT4_MAC + INT8X4
...
// Execution path:
//
//   Vector and matrix register banks
//                  ↓
//       Mixed-precision SIMD8 engine
//                  ↓
//     Eight shared FP32 accumulators
//
// Unsupported commands and invalid source operands are consumed as rejected
// commands and never reach the execution datapath.
// -----------------------------------------------------------------------------

module nce_mixed_precision_command_core #(
    ...
    logic execution_precision_supported;
    ...
    // -------------------------------------------------------------------------
    // Command validation and issue
    // -------------------------------------------------------------------------

    nce_mixed_precision_command_decoder #(
    ...
    // -------------------------------------------------------------------------
    // Register-bank and SIMD execution
    // -------------------------------------------------------------------------

    nce_mixed_precision_register_mac_core #(
        .INT8X4_PRECISION (INT8X4_PRECISION),
        .BF16X2_PRECISION (BF16X2_PRECISION),
        .BF24_PRECISION   (BF24_PRECISION)
    ) u_execution_core (
        ...
        .exec_precision_i            (cmd_precision_i),
    ...
    // The decoder rejects unsupported precision before execution. This signal
    // is retained as an internal consistency check and does not feed back into
    // command readiness, avoiding a combinational control loop.

    /* verilator lint_off UNUSEDSIGNAL */
    logic execution_precision_supported_unused;

    assign execution_precision_supported_unused =
        execution_precision_supported;
