...
module tb_nce_bf16_mac_lane;
    ...
    logic [15:0] vector_lhs;
    logic [15:0] vector_rhs;
    logic [31:0] expected_accumulator;

    nce_bf16_mac_lane dut (
        .clk_i                (clk_i),
        .rst_ni               (rst_ni),
    ...
    task automatic issue_operation (
        input logic [15:0] operation_lhs,
        input logic [15:0] operation_rhs,
            ...
            for (
                ...
                if (
                    ...
                    if (
                        ...
                        if (error_count <= 20) begin
                            $display(
                                "  lhs=%04h rhs=%04h result=%08h expected=%08h",
                                operation_lhs,
                                operation_rhs,
                                accumulator_o,
                                expected_result
                    ...
                    if (
                        ...
                        report_error(
                            "Accumulator valid was not asserted"
                        );
                    end
    ...
    initial begin
        ...
        if (
            ...
            $fatal(
                1,
                "BF16 MAC lane reset state is incorrect."
            );
        end
        ...
        if (vector_file == 0) begin
            $fatal(
                1,
                "Unable to open BF16 MAC vector file."
            );
        end
        ...
        // ---------------------------------------------------------------------
        // Directed exception propagation tests
        // ---------------------------------------------------------------------

        apply_clear();

        // +infinity × zero -> canonical quiet NaN and invalid.
        issue_operation(
            16'h7F80,
            16'h0000,
            32'h7FC0_0000,
        ...
        // Largest finite BF16 × largest finite BF16 -> FP32 infinity.
        issue_operation(
            16'h7F7F,
            16'h7F7F,
            32'h7F80_0000,
        ...
        // Smallest BF16 subnormal squared -> rounded FP32 zero.
        issue_operation(
            16'h0001,
            16'h0001,
            32'h0000_0000,
        ...
        if (error_count == 0) begin
            $display(
                "PASS: nce_bf16_mac_lane passed %0d vectors and %0d completed operations.",
                vector_count,
                operation_count
            );
        end
            ...
            $fatal(
                1,
                "FAIL: %0d errors detected in BF16 MAC lane verification.",
                error_count
            );
        end
