...
python3 -m py_compile \
    ...
    scripts/gen_mixed_precision_simd8_vectors.py

echo "===== Generate mixed-precision SIMD8 vectors ====="
python3 scripts/gen_mixed_precision_simd8_vectors.py \
    --output build/mixed_precision_simd8_vectors.txt \
    --random-count 4000
...
iverilog \
    -g2012 \
    -Wall \
    -s tb_nce_mixed_precision_simd8_mac \
    -o build/tb_nce_mixed_precision_simd8_mac.vvp \
    "${RTL_FILES[@]}" \
    tb/unit/tb_nce_mixed_precision_simd8_mac.sv

echo "===== Mixed-precision SIMD8 simulation ====="
vvp build/tb_nce_mixed_precision_simd8_mac.vvp \
    | tee reports/tb_nce_mixed_precision_simd8_mac.log
...
verilator \
    ...
    -Wno-fatal \
    -Wno-PINCONNECTEMPTY \
    --top-module nce_mixed_precision_simd8_mac \
    "${RTL_FILES[@]}"

echo "===== Yosys synthesis check ====="
yosys -q -p "
    read_verilog -sv ${RTL_FILES[*]};
    hierarchy -check -top nce_mixed_precision_simd8_mac;
    proc;
    opt;
    check -assert;
    stat;
" | tee reports/yosys_nce_mixed_precision_simd8_mac.log

echo "===== All mixed-precision SIMD8 checks completed ====="
