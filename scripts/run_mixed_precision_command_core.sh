...
if [[ -z "${REGFILE_FILE}" ]]; then
    echo "ERROR: Unable to locate nce_regfile_16x256."
    exit 1
fi

if [[ -z "${REGISTER_BANKS_FILE}" ]]; then
    echo "ERROR: Unable to locate nce_register_banks."
    exit 1
fi
...
iverilog \
    -g2012 \
    -Wall \
    -s tb_nce_mixed_precision_command_core \
    -o build/tb_nce_mixed_precision_command_core.vvp \
    "${RTL_FILES[@]}" \
    tb/unit/tb_nce_mixed_precision_command_core.sv

echo "===== Mixed-precision command-core simulation ====="
vvp build/tb_nce_mixed_precision_command_core.vvp \
    | tee reports/tb_nce_mixed_precision_command_core.log
...
verilator \
    ...
    -Wno-fatal \
    -Wno-PINCONNECTEMPTY \
    --top-module nce_mixed_precision_command_core \
    "${RTL_FILES[@]}"

echo "===== Yosys synthesis check ====="
yosys -q -p "
    read_verilog -sv ${RTL_FILES[*]};
    hierarchy -check -top nce_mixed_precision_command_core;
    proc;
    opt;
    check -assert;
    stat;
" | tee reports/yosys_nce_mixed_precision_command_core.log

echo "===== All mixed-precision command-core checks completed ====="
